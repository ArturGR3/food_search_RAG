...
This project implements a recipe recommendation system that helps users find recipes based on their queries. The system utilizes RAG (Retrieval-Augmented Generation) technique where the system first adjusts user queries, retrieves relevance recipes from LanceDB (multi-modal database) using FTS (Full-Text Search), after than relevant recipes are sorted based on relevance score using LLM as a judge and only high relevant recipes are returned to a user. By combining advanced search capabilities with a user-friendly interface, this tool aims to simplify the process of discovering new recipes and meal ideas.
...
2. [Generating Synthetic Data](#2-generating-synthetic-data-to-evaluate-retrieval-methods) - Synthetic data is generated for evaluation of retrieval methods
...
7. [Data Collection for Monitoring](#7-data-collection-for-monitoring) - All steps from user query to user clicks are logged into SQLite database
...
11. [Jinja Templating](#11-jinja-templating) - Creates prompts and tracks versions
12. [LLM Factory](#12-llm-factory) - Creates LLM client with different APIs (inspired by [this YouTube video](https://www.youtube.com/watch?v=p6mkittu8Zc))
13. [Instructor Library](#13-instructor-library) - Structures output from LLM
14. [Evaluation Criteria](#14-evaluation-criteria) - Defines evaluation criteria for the project

### 1. Data Collection and Preprocessing

The data collection and preprocessing steps are handled automatically using two Python scripts:

1. Data Collection (`setup_data.py`):
   We use the Kaggle API to download the recipe dataset from a Kaggle competition [1]. The dataset contains food ingredients and recipes with image name mappings, providing a rich source of culinary information for our recipe recommendation system. The `setup_data.py` script performs the following tasks:
...
2. Data Preprocessing (`data_preprocessing.py`):
   After the data is downloaded and extracted, we use `data_preprocessing.py` to process the raw data. This script performs the following tasks:
...
In the early stage of the application when we dont have user interaction data, to evaluate the initial retrieval performance of our system, we generate synthetic data in the form of user-like prompts for each recipe. This process is handled by the [ground_truth_generation.py](./src/data_preprocessing/ground_truth_generation.py) script.
...
Once we generated synthetic data, we can evaluate the performance of different retrieval methods on how well they can retrieve the original recipe based on user prompts. The evaluation metrics we use are Mean Reciprocal Rank (MRR) and Recall for top 1, 3, and 5 results. We use [comparison_retrieval.py](./src/retrieval/comparison_retrieval.py) script to evaluate the performance of different retrieval methods such as FAISS, LanceDB Vector, LanceDB FTS and LanceDB Hybrid.
...
During testing of the application we found out that the FTS does not perform well on negation queries (e.g. 'soup without onions') as well as when there is spelling mistakes in the user query (e.g. 'soup with oniins'). To address this we implemented a user query preprocessing step that rewrites the user query to improve the recall of the system. This is done using [query_preprocessor.py](./src/user_query_processing/query_preprocessor.py) script. Our approach is as follows:
...
In this step we use LLM as a judge to score and filter retrieved recipes based on user query match. This is done using [recipe_recommendation.py](./recipe_recommendation.py) script. In this call we ask LLM to score each recipe based on how well it matches the user query from 0 to 10 as well as to provide a reason for the score. We use this assessment to further filter out low-relevance recipes and log the reasons for low scores to help improve the model in the future.
...
All the steps from user query to user clicks are logged into SQLite database. This allows us to monitor the performance of the system and to log the user feedback for future reference. The script [db.py](./src/reporting/db.py) is used to initialize the database, create tables and functions to log the data from the application.

### 8. Monitoring

For this stage of the application we have created a simple one-pager summary of the results using [performance_report.py](./src/reporting/performance_report.py) script. This script creates a summary of the results and saves it as a PNG image. You can reffer to example of the report [here](./src/reporting/performance_report.png). Here we measure the likes/dislikes, what is the relevance score distribution and what are the most common recipes that user liked or disliked.
...
Explanation: Refer to the [Introduction](#introduction) section above.
...
Explanation: Please refer to the RAG schema in the [Introduction](#introduction) section above.
...
Explanation: Please refer to the [Retrieval Method Comparison](#retrieval-method-comparison) section above. FAISS semantic search, LanceDB FTS, vector search and hybrid search were evaluated.
...
Explanation: Please refer to the [RAG Evaluation](#rag-evaluation) section above. User query preprocessing were evaluated, to tackle recipe negation challenge (eg. for 'soup without onions' text search might return 'soup with onions').
...
Explanation: Gradio interface was built to interact with the application.
...
Explanation: Data preprocessing pipeline was built to ingest the dataset into the knowledge LanceDB. This is orchestrated using Makefile.
...
Explanation: User feedback is collected in SQLLite database and python script is used to create a simple one-pager summary of the results.
...
Explanation: Docker file is provided for the main application and orchestrated using makefile.
...
Explanation: Comprehensive instructions are provided in the README file.
...
Explanation: Hyrbrid search was evaluated, however it was not significantly better. Refer to the [Retrieval Method Comparison](#retrieval-method-comparison) section above. Document re-ranking is used as LLM as a judge. User query rewriting is used to tackle recipe negation challenge.

### Bonus Points (not covered in the course)
- [ ] Deployment to the cloud (2 points)
- [X] Up to 3 extra bonus points for additional features (please specify):

Explanation: Jinja templating was used to create the promt formating. LLM Factory was used to create the LLM client with different APIs. Asyncio was used to handle concurrent API calls.